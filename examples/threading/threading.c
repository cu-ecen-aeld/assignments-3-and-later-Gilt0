...
void* threadfunc(void* thread_param)
{
    ...
    int wait_to_obtain_ms = (thread_func_args -> wait_to_obtain_ms)*1000;
    int wait_to_release_ms = (thread_func_args -> wait_to_release_ms)*1000;
    ...
    thread_func_args -> thread_complete_success = (lock_status == 0) & (unlock_status == 0);
    return thread_param;
}
...
bool start_thread_obtaining_mutex(pthread_t *thread, pthread_mutex_t *mutex,int wait_to_obtain_ms, int wait_to_release_ms)
{
    ...
    int create_status = pthread_create(thread, NULL, threadfunc, (void*) thread_param);
    thread_param -> thread_complete_success = (create_status == 0);
